...
--
-- Dumping routines for database 'blood_donation_managment_system'
--
...
if stat='available' then
    ...
    insert into delivered_blood(order_date,hospital_id_fk , bank_id_fk,bag_id_fk)
    values(cur_date,d_hospital_id, d_bank_id,d_bag_id);
    else 
    select ' this blood bag is already delivered to hospital' as alert;
    end if;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `donor_registration`(in d_name varchar(45),in d_cnic varchar(45),in d_contact_no varchar(45),in d_address varchar(100),in d_email varchar(45),in d_age int ,in d_gender varchar(45), in d_staff_id_fk int )
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_blood_bank`(in b_bank_name varchar(45),in b_province varchar(45),in b_city varchar(45),in b_location varchar(45),in b_contact_no varchar(45),in b_email varchar(45),in b_manager_id int ,in b_existance_date date)
BEGIN
    declare loc int;
    declare contact_len int;
    ...
    insert into blood_banks(bank_name , province , city , location , contact_no ,email,existance_date,manager_id_fk )
    values(b_bank_name , b_province, b_city ,b_location ,b_contact_no ,b_email,b_existance_date,b_manager_id);
else 
select "some fields are incorrect" as message;  
end if;
END ;;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_blood_test`(in t_test_name varchar(45), in t_test_result varchar(45), in t_bag_id_fk int ,in t_blood_type varchar(45)  )
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_donation`(in b_amount varchar(45),in b_donor_id_fk int,in b_bank_id_fk int)
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_hospital`(in h_hospital_name varchar(45),in h_province varchar(45),in h_city varchar(45),in h_location varchar(45),in h_contact_no varchar(45),in h_email varchar(45) )
BEGIN
    declare contact_len int;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_manager`(in m_name varchar(45),in m_phone varchar(45),in m_cnic varchar(45),in m_email varchar(45),in m_address varchar(100),in m_salary float,in m_joining_date date)
BEGIN
declare loc int;
declare cnic_len int;
...
BEGIN
    ...
    insert into organiz_compaigns( bank_id_fk, compaign_no_fk)
...
BEGIN
    ...
    insert into participated_staff(staff_id_fk , compaign_no_fk)
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `new_staff_member`(in s_name varchar(45),in s_cnic varchar(45),in s_phone varchar(45),in s_position varchar(45),in s_joining_date date,in s_manager_id int , in s_bank_id int,in s_address varchar(100),in s_salary float)
BEGIN
    declare phone_len int;
    declare cnic_len int;
    ...
    if (cnic_len=13 and  phone_len=11) then 
    insert into staff_members(name , cnic , phone , position , joining_date ,manager_id_fk,bank_id_fk, address , salary)
